...
export interface Spec extends TurboModule {
  configure: (config: BackgroundFetchConfig, success: (status: BackgroundFetchStatus) => void, failure: (status: BackgroundFetchStatus) => void) => void;
  scheduleTask: (config: TaskConfig, success: (status: boolean) => void, failure: (status: boolean) => void) => void;
  start: (success: (status: BackgroundFetchStatus) => void, failure: (status: BackgroundFetchStatus) => void) => void;
  stop: (taskId: string, success: (status: boolean) => void, failure: (status: boolean) => void) => void;
  status: (success: (value: BackgroundFetchStatus) => void) => void;
  finish: (taskId: string) => void;
  addListener: () => void;
  removeListeners: () => void;
}

export default TurboModuleRegistry.get<Spec>('RNBackgroundFetch') as Spec | null;